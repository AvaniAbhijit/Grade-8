...
#Task 1: Create the box widgets when "Play with your own button is pressed"
    #Label: "Enter your secret word:"
    #Text Input: "Enter Your Word Here" - This should point to function: update_secret_word_handler
    #Button: "Start game" - This should point to function: start_new_game
#Task 2: Add text_input focus so that the cursor doesn't need to be set everytime
...
class Word(toga.App):
    ...
    def process_letter(self, widget): #function to check if only letters are entered
        ...
        if lowercase in Word.valid_alphabet: #if it is present in the alphabet list defined by user
            if lowercase in self.secret_letters: #if secret letter is in word, replace underscore with letter
    ...
    def enter_own_word(self,widget): #define function to allow user to enter the word
        ...
        #Open given link for expected output.Copy and paste the given url in the browser to observe the output.
        #(https://drive.google.com/file/d/1vuLMRSgiZEkoS7Bj933vhHDIj2vmaNZs/view?usp=drive_link)
        #add your code here
