...
class LoginApp(toga.App):
    title_style=Pack(text_align="center",font_size=20,font_weight="bold", color="blue", padding=10)
    ...
    text_style2=Pack(text_align="center", color="green", font_size=15, padding=5, font_weight="bold")
    
    def startup(self):
        self.main_window = toga.MainWindow(title='QuizApp')

        label1 = toga.Label("Ready for QUIZ", style=LoginApp.title_style)
    ...
    def show_quiz(self,username):
        ...
        message_label = toga.Label(f'Welcome to the Quiz {username}!', style=LoginApp.title_style)
        ...
        close_button = toga.Button("Close", on_press=self.close_win, style=LoginApp.button_style)
        box1.add(message_label)

        # Display score
        self.score_label = toga.Label(f'Score: {self.score}', style=LoginApp.text_style2)
