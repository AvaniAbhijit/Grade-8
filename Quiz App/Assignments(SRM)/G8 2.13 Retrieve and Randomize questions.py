...
class LoginApp(toga.App):
    ...
    def display_selected_option(self, widget): #function to identify the selected option
        ...
        #get the reference to the subject - for example CS
        ques_ref = db.reference(ques_str)

        #fetch random 2 questions from the list of questions under CS
        self.ques = random.sample(list(ques_ref.get().items()),2)
        print(self.ques)

        #set the index of the first question as 0
        ...
        #Add other options similar to previous

        select = toga.Selection(items=["Select an option", "option : 1","option : 2","option : 3","option : 4"],on_change=self.check_answer)
        select.value = "Select an option"
        self.box2.add(select)
